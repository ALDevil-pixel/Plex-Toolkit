...
output="$(PATH="$TMP/bin:$PATH" "$ROOT/plex-toolkit" plex-sync-plan \
    --config "$TMP/plex.conf" "$TMP/Movies" 1)"

grep 'ADD_TO_PLEX' <<<"$output" >/dev/null
grep 'Local Only (2022).mp4' <<<"$output" >/dev/null
grep $'NONE\tMATCH' <<<"$output" >/dev/null
...
test -f "$TMP/Movies/notes.txt"

if PATH="$TMP/bin:$PATH" "$ROOT/plex-toolkit" plex-sync-plan \
    --config "$TMP/plex.conf" --fix "$TMP/Movies" 1 >/dev/null 2>&1; then
    exit 1
fi
